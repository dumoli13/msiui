...
function Steps({ active, items, onChange, disabled = false, }) {
    ...
    return (_jsx("div", { className: "flex items-start justify-between gap-4 w-full", children: items.map((item, index) => {
            ...
            if (active === index) {
                ...
                return (_jsxs("div", { className: cx(`flex gap-2 relative flex-1 rounded-2xl ${!disabled && available ? 'hover:bg-neutral-20 dark:hover:bg-neutral-20-dark cursor-pointer' : 'cursor-default'}`, {
                        'items-start': !!description,
                        'items-center': !description,
                    }), role: "button", onClick: () => {
                        if (available)
