...
export function FormField({ children }) {
    ...
    return (enhanceChild?.(children) ?? children);
}
// ──────────────────────────────────────────────────────────────────────────────
/**
...
const isFormInput = (el) => React.isValidElement(el) &&
    !!el.type.isFormInput;
const isFormSubmitButton = (el) => {
    return (React.isValidElement(el) &&
        el.props.type === 'submit');
};
const FormFieldWrapper = React.memo(function FormFieldWrapper({ name, errorStore, disabledStore, propDisabled, externalDisabled, children, }) {
    const error = React.useSyncExternalStore(errorStore.makeSubscribe(name), errorStore.makeSnapshot(name));
    const isSubmitting = React.useSyncExternalStore(disabledStore.subscribe, disabledStore.getSnapshot);
...
const FormSubmitButtonWrapper = React.memo(function FormSubmitButtonWrapper({ disabledStore, externalDisabled, children, }) {
    const isSubmitting = React.useSyncExternalStore(disabledStore.subscribe, disabledStore.getSnapshot);
 ...
 */
const Form = ({ onSubmit, onError, onReset, className, rules, disabled = false, formRef, submitOnChange = false, focusOnLastFieldEnter = false, children, template, }) => {
        ...
        for (const [fieldName, fieldRules] of Object.entries(rules(typedValues))) {
            ...
            for (const element of refs) {
                ...
                for (const rule of fieldRules) {
                    const checkValue = (innerVal, innerInstanceKey) => {
                        ...
                        if (rule.required &&
                            ...
                            const pattern = typeof rule.pattern === 'string'
                                ...
                                : rule.pattern;
                            const str = typeof innerVal === 'string' || typeof innerVal === 'number'
                                ? String(innerVal)
                                : '';
                            if (!pattern.test(str)) {
                            ...
                            newErrors[innerInstanceKey] = getErrorMessage(rule, 'exactLength');
                        }
                    ...
                    if (newErrors[instanceKey])
                        break; // Stop at first error for this instance
                }
            }
        }
    ...
    const getValue = React.useCallback((key) => store.getValue(key), [store]);
    const getValues = React.useCallback(() => store.getValues(), [store]);
    ...
    const handleFormKeyDown = React.useCallback((e, currentKey) => {
    ...
    const enhanceChild = React.useCallback((child) => {
        ...
        if (isFormSubmitButton(child)) {
            ...
            return (_jsx(FormSubmitButtonWrapper, { disabledStore: store.disabledStore, externalDisabled: disabledRef.current, children: cloned }));
        }
        ...
        if (isFormInput(child)) {
            ...
            const errorKey = store.renderCounters[name] === 0
                ? name
            ...
            return (_jsx(FormFieldWrapper, { name: errorKey, errorStore: store.errorStore, disabledStore: store.disabledStore, propDisabled: childProps.disabled, externalDisabled: disabledRef.current, children: enhancedChild }, errorKey));
        }
    ...
    const formContextValue = React.useMemo(() => ({ enhanceChild: (child) => enhanceChildRef.current(child) }), []);
    ...
    const renderTemplate = React.useCallback((formTemplate) => {
        ...
        const wrapWithErrorStore = (name, el, propDisabled) => {
            if (!name)
                return el;
            return (_jsx(FormFieldWrapper, { name: name, errorStore: store.errorStore, disabledStore: store.disabledStore, propDisabled: propDisabled, externalDisabled: disabledRef.current, children: el }, el.key ?? undefined));
        };
        ...
        const renderItem = (item, index) => {
            ...
            switch (item.component) {
                ...
                case 'AutoComplete':
                    return wrapWithErrorStore(item.name, _jsx(AutoComplete, { ...item, ...commonInputProps(item.name, item.onChange) }, key), item.disabled);
                case 'AutoCompleteMultiple':
                    return wrapWithErrorStore(item.name, _jsx(AutoCompleteMultiple, { ...item, ...commonInputProps(item.name, item.onChange) }, key), item.disabled);
                case 'Checkbox':
                    return wrapWithErrorStore(item.name, _jsx(Checkbox, { ...item, ...commonInputProps(item.name, item.onChange) }, key), item.disabled);
                case 'DatePicker':
                    return wrapWithErrorStore(item.name, _jsx(DatePicker, { ...item, ...commonInputProps(item.name, item.onChange) }, key), item.disabled);
                case 'DateRangePicker':
                    return wrapWithErrorStore(item.name, _jsx(DateRangePicker, { ...item, ...commonInputProps(item.name, item.onChange) }, key), item.disabled);
                case 'MultipleDatePicker':
                    return wrapWithErrorStore(item.name, _jsx(MultipleDatePicker, { ...item, ...commonInputProps(item.name, item.onChange) }, key), item.disabled);
                case 'NumberTextField':
                    return wrapWithErrorStore(item.name, _jsx(NumberTextField, { ...item, ...commonInputProps(item.name, item.onChange) }, key), item.disabled);
                case 'PasswordField':
                    return wrapWithErrorStore(item.name, _jsx(PasswordField, { ...item, ...commonInputProps(item.name, item.onChange) }, key), item.disabled);
                case 'RadioGroup':
                    return wrapWithErrorStore(item.name, _jsx(RadioGroup, { ...item, ...commonInputProps(item.name, item.onChange) }, key), item.disabled);
                case 'Select':
                    return wrapWithErrorStore(item.name, _jsx(Select, { ...item, ...commonInputProps(item.name, item.onChange) }, key), item.disabled);
                case 'Switch':
                    return wrapWithErrorStore(item.name, _jsx(Switch, { ...item, ...commonInputProps(item.name, item.onChange) }, key), item.disabled);
                case 'TextArea':
                    return wrapWithErrorStore(item.name, _jsx(TextArea, { ...item, ...commonInputProps(item.name, item.onChange) }, key), item.disabled);
                case 'TextField':
                    return wrapWithErrorStore(item.name, _jsx(TextField, { ...item, ...commonInputProps(item.name, item.onChange) }, key), item.disabled);
                case 'TimerField':
                    return wrapWithErrorStore(item.name, _jsx(TimerField, { ...item, ...commonInputProps(item.name, item.onChange) }, key), item.disabled);
                default:
    ...
    const enhancedChildren = React.useMemo(() => React.Children.map(children, enhanceChild), [children, enhanceChild]);
    return (_jsx(FormContext.Provider, { value: formContextValue, children: _jsx("form", { className: className, onSubmit: (e) => {
                e.preventDefault();
                handleSubmitRef.current();
            }, onReset: (e) => {
                e.preventDefault();
                handleReset();
            }, children: template ? renderTemplate(template) : enhancedChildren }) }));
};
export default Form;
