 ...
 *
 */
const InputDropdown = ({ id, open, children, elementRef, dropdownRef, fullWidth, maxHeight = 300, animation, }) => {
    const { container: overlayContainer } = React.useContext(OverlayContext);
    const prevOpenRef = React.useRef(false);
    const [isClosing, setIsClosing] = React.useState(false);
    const { animDuration, animEase, animDurationOut, animEaseOut } = resolveAnimation(animation, {
    ...
    return (_jsx(Portal, { children: _jsx("div", { id: open ? id : undefined, role: "none", tabIndex: -1, onMouseDown: (e) => e.stopPropagation(), ref: dropdownRef, style: {
                top: 0,
                left: 0,
                transform: `translate(${position.left}px, ${position.top}px)`,
                width: position.width,
                maxHeight,
            }, className: cx('absolute z-[2300] bg-neutral-10 dark:bg-neutral-10-dark shadow-box-2 rounded-lg py-1.5 text-neutral-100 dark:text-neutral-100-dark overflow-y-auto cursor-default', { invisible: !open && !isClosing }), onKeyDown: (e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                    e.stopPropagation();
                }
            }, children: open || isClosing ? children : null }) }));
};
