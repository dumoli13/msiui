 ...
 */
const Checkbox = (_a) => {
    var { label = '', labelPosition = 'top', checked: valueProp, defaultChecked = false, indeterminate = false, onChange, helperText, disabled, className, inputRef, error: errorProp, width, 'aria-label': ariaLabel } = _a, props = __rest(_a, ["label", "labelPosition", "checked", "defaultChecked", "indeterminate", "onChange", "helperText", "disabled", "className", "inputRef", "error", "width", 'aria-label']);
    ...
    return (React.createElement("div", { className: cx({ 'flex items-center gap-4': labelPosition === 'left' }, className), style: width ? { width } : undefined },
        React.createElement("label", { className: cx('inline-flex gap-2 w-fit', {
                'cursor-not-allowed opacity-50': disabled,
                'cursor-pointer': !disabled,
            }) },
            React.createElement("div", { role: "checkbox", "aria-checked": "true", "aria-disabled": "false", "aria-label": ariaLabel, className: cx('shrink-0 w-5 h-5 rounded-md border flex justify-center items-center transition-all box-border relative', {
                    'bg-neutral-20 border-neutral-40': disabled,
                    'border-neutral-50 bg-neutral-10 hover:border-primary-main': !disabled,
                    'bg-primary-main border-primary-main': !disabled && value && !indeterminate,
