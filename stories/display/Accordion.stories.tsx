...
import { Accordion, AccordionProps, Button } from '../../src';
...
const meta: Meta<AccordionProps> = {
  ...
  argTypes: {
    ...
    defaultActiveKey: {
      control: { type: 'object' },
      description:
        'The keys of the panels that should be open by default on initial render.',
      table: {
    ...
    activeKey: {
      ...
      description:
        'Controlled keys of the currently active panels, for external control.',
      table: {
        type: { summary: 'string[] | number[]' },
      },
    },
...
const items = [
  ...
  },
  {
    key: '3',
    title: 'Third Item',
    content: 'Another piece of content for the third.',
  },
];
    ...
    <div className="flex flex-col gap-4 w-full max-w-md">
      {sizeOption.map((size) => (
        <Accordion
          key={size}
          {...args}
          size={size as AccordionProps['size']}
          items={[
            {
              key: '1',
              title: `Size ${size}`,
              content: `Content for ${size} size accordion.`,
            },
          ]}
        />
      ))}
...
export const ControlledAccordion: Story = {
  render: (args) => {
    const [activeKey, setActiveKey] = React.useState<Array<string | number>>([
      '1',
    ]);
    ...
    return (
      <div className="flex flex-col gap-4 w-full ">
        ...
        <div className="flex gap-2 justify-center">
          <Button onClick={() => setActiveKey(['1'])}>Open First</Button>
          <Button onClick={() => setActiveKey(['2'])}>Open Second</Button>
          <Button onClick={() => setActiveKey(['3'])}>Open Third</Button>
          <Button variant="secondary" onClick={() => setActiveKey([])}>
            Close All
          </Button>
          <Button
            variant="secondary"
  ...
  args: {
    items: [
      {
        key: '1',
        title: 'Controlled First Item',
        content: 'This is the first controlled item.',
      },
      {
        key: '2',
        title: 'Controlled Second Item',
        content: 'This is the second controlled item.',
      },
      {
        key: '3',
        title: 'Controlled Third Item',
        content: 'This is the third controlled item.',
      },
    ],
    collapsible: 'header',
    size: 'default',
