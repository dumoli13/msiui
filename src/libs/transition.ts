...
export const DEFAULT_ANIMATION_IN: Required<AnimationDefaults> = {
  duration: 0.3,
  ease: 'back.out(1.4)',
  transition: 'grow',
};

export const DEFAULT_ANIMATION_OUT: Required<AnimationDefaults> = {
  duration: 0.25,
  ease: 'power2.in',
  transition: 'grow',
};

export const createSymmetricAnimation = (
  anim: ModalAnimation,
): ModalAnimationConfig => anim;

export const resolveAnimation = (
  animation?: ModalAnimationConfig,
  defaults?: { in?: AnimationDefaults; out?: AnimationDefaults },
) => {
  ...
  return {
    ...
    animTransformOrigin: (inAnim as { transformOrigin?: string } | undefined)
      ?.transformOrigin,
    ...
    animTransformOriginOut: (
      outAnim as { transformOrigin?: string } | undefined
    )?.transformOrigin,
  };
};
...
const getTransitionVars = (
  transition: ModalTransition = 'zoom',
  transformOriginIn?: string,
  transformOriginOut?: string,
): TransitionVars => {
  const transitions: { [key in ModalTransition]: TransitionVars } = {
    fade: {
      from: { opacity: 0 },
      to: { opacity: 1 },
      fromOut: { opacity: 0 },
